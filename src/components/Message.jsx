...
export default function Message({ message, isYou }) {
  const countyCode = message.country && message.country !== "undefined" ? message.country.toLowerCase() : "";
  
  return <Box display="grid" justifyItems={isYou ? "end" : "start"}>
    <Grid
    ...
    >
      <GridItem fontWeight="500" fontSize="md" justifySelf="start" color="gray.500" mb="2">
        ...
        {message.is_authenticated && ( <MdVerified color="#1d9bf0" style={{ display: "inline", marginRight: "5px" }}/> )}
        
        {countyCode && <Box display="inline-block" fontSize="10px"> from {message.country}{" "}</Box>}
      </GridItem>
      
      <GridItem justifySelf="start" textAlign="left" wordBreak="break-word" fontSize="md" fontFamily="Montserrat, sans-serif">
